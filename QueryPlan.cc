...
void QueryPlan::print(std::ostream& os) const {
    //cout << "QueryPlan :: print " << endl;
  root->print(os);
  //cout << "QueryPlan :: print done" << endl;
}
...
void QueryPlan::execute() {
  outFile = (outName == "STDOUT" ? stdout
    ...
    //cout << "outputFile:" << outFile << outName.c_str() << endl;
  if (outFile) {

    //cout << "outFile Rohit" << numNodes << endl;
    int numNodes = root->pipeId;
    Pipe** pipes = new Pipe*[numNodes];
    RelationalOp** relops = new RelationalOp*[numNodes];
        //cout << "outFile Rohit" << numNodes << endl;
        //cout << "outFile root->execute(pipes, relops);" << outFile << endl;
...
//From here we are constructing the nodes
